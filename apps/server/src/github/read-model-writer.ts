...
import type { Pipeline, RepoFlow } from "@sphynx/schema/review-queue";
...
const makeReadModelWriter = Effect.gen(function* () {
  ...
  const writeRepo = async (
    ...
    const repoId = repoRowId(installationId, flow.owner, flow.repo);
    await tx
      .insert(reviewRepo)
      .values({
        id: repoId,
      ...
      .onConflictDoUpdate({
        ...
        set: { stages: [...flow.stages], updatedAt: now },
      });
